#include <stdio.h>
#include <stdbool.h>
#include <string.h>
#define BUFFER_SIZE 1048576
#define NAME_SIZE 1024
const char key[] = {
	
};
const char uwpHead[] = "QQMu";
const char mp3Head[] = "ID3";
const char flacHead[] = "fLaC";
const char oggHead[] = "OggS";
char buffer[BUFFER_SIZE];
char name[NAME_SIZE];

char getKey(size_t i) {
	if(i >= 0) {
		if(i > 0x7FFF)
			i %= 0x7FFF;
	} else i = 0;
	return key[(i * i + 80923) % 256];
}

int main(int argc, char **argv) {
	if(argc == 1) {
		printf("Please drag in file\n");
		system("pause");
		return 0;
	}
	int fileCount;
	size_t i, j, len;
	for(fileCount = 1; fileCount < argc; fileCount++) {
		strcpy(name, argv[fileCount]);
		FILE *in = fopen(name, "rb");
		if(!in) {
			printf("Error: failed to open %s\n", name);
			continue;
		}
		printf("%s --> ", name);

		//read head and rename
		char *dot = strrchr(name, '.');
		if(dot)
			dot = '\0';

		fread(buffer, 4, 1, in);
		buffer[4] = '\0';
		if(strcmp(buffer, uwpHead) == 0) {
			fseek(in, 0x100, SEEK_SET);
			fread(buffer, 4, 1, in);
		}

		for(i = 0; i < 4; i++)
			buffer[i] ^= getKey(i);
		buffer[4] = '\0';
		if(strcmp(buffer, mp3Head) == 0)
			strcat(name, ".mp3");
		else if(strcmp(buffer, flacHead) == 0)
			strcat(name, ".flac");
		else if(strcmp(buffer, oggHead) == 0)
			strcat(name, ".ogg");
		else {
			strcat(name, ".m4a");
			printf("Warning: decryption may fail.\n");
		}

		FILE *out = fopen(name, "wb");
		if(!out) {
			printf("Error: failed to write %s\n", buffer);
			continue;
		}

		fwrite(buffer, 4, 1, out);

		//decrypt
		size_t count, front = 4;
		while(count = fread(buffer, 1, BUFFER_SIZE, in), count != 0) {
			for(i = 0; i < count; i++)
				buffer[i] ^= getKey(front + i);
			fwrite(buffer, count, 1, out);
			front += count;
		}
		printf("Success\n");
		fclose(in);
		fclose(out);
	}
	return 0;
}
